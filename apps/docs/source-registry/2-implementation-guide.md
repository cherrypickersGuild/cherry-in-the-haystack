...
**칼럼 삭제는 만들지 않는다(D10).** 칼럼 하나를 지우면 모든 행의 값이 한꺼번에 안 보이게 된다 — 행 하나를 지우는 것과 무게가 다르다.

**행을 지우기 전에 한 번 물어본다(D10).** 확인 모달에 무엇이 사라지는지 **이름과 영향**을 적는다 — 목업 그대로.

```
이 행을 지울까요?   Dwarkesh Patel 을(를) 지웁니다.
                    화면에서 사라지지만 DB 에는 남습니다 — 되살릴 수 있습니다.   [취소] [행 삭제]
```

**칼럼 이름을 바꾸는 것은 머리줄의 `⋯` 메뉴**에 둔다. 칼럼마다 아이콘을 늘어놓으면 머리줄이 아이콘 밭이 된다.
