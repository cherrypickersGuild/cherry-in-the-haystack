...
### 3) PDF extractor의 기존 프로토타입 DB write
...
summary, why_it_matters, evidence, related_concepts, references, updates, patch_notes
...
| related_concepts | company_article_ai_state.ai_entities_json.concepts | JSONB object 내부 array | (1번 참조) |
| related_concepts | company_article_ai_state.ai_structured_extraction_json.related_concepts | JSONB object 내부 array | (1번 참조) |
| updates | company_article_ai_state.ai_structured_extraction_json.content_updates.updates | JSONB object 내부 array | (1번 참조) |
| patch_notes | company_article_ai_state.ai_structured_extraction_json.content_updates.change_notes | JSONB object 내부 array | (1번 참조) |

### 주의

- Writer의 patch_notes는 DDL Patchnotes(미읽음 집계)와 **다른 개념**이다.
...
title, summary, importance_score, category, newsletter_fit, tags, company

### 즉시 할당표

| News Agent 산출물 | DDL 컬럼 | 타입 | 참조 |
| --- | --- | --- | --- |
...
| tags | company_article_ai_state.ai_tags_json | JSONB array | (2번 참조) |
| company | company_article_ai_state.ai_entities_json.companies | JSONB object 내부 array | (2번 참조) |
| newsletter_fit | company_article_ai_state.ai_structured_extraction_json.newsletter_fit | JSONB object 내부 boolean | (2번 참조) |

### 점수 변환식

importance_score(1~5) -> ai_score(0~100) 변환

권장 변환식:

ai_score = importance_score * 20
...
books, paragraph_chunks, key_ideas

### 역할

- paragraph_chunks/key_ideas는 기사 AI가 근거 생성 시 참조 소스로 사용 가능
- DDL 직접 매핑 대상이라기보다, ai_evidence_json.paragraph_evidence 생성의 upstream 데이터 소스
    
    (JSONB object 내부 array, 3번 참조, 4번 참조)
...
| --- | --- |
| id | 운영키/조인키(집계 생성) |
| company_id | 운영키/조인키(집계 생성) |
| company_article_state_id | 운영키/조인키(집계 생성) |
...
| created_at | DB 기본값/트리거 |
| updated_at | DB 기본값/트리거 |
...
| company_id | 집계 생성 |
| stat_date | 집계 생성 |
| category_group_id | 2번 JSON (category) + 일자 기준 집계 생성 |
| category_id | 2번 JSON (category) + 일자 기준 집계 생성 |
| article_count | 집계 생성(일별 카테고리 건수) |
| high_impact_count | 집계 생성(고임팩트 건수) |
| score_sum | 2번 JSON (importance_score) 변환 후 집계 생성 |
| score_avg | 2번 JSON (importance_score) 변환 후 집계 생성 |
...
- 필요 시 추후 변환/백필 배치로 company_article_ai_state에 반영
...
D:/intellij project/cherry-in-the-haystack/dev/apps/agent/writer_agent/run_writer_agent.py

### 이 JSON은 무엇인가

기사 1건을 깊게 가공한 Writer 결과 JSON(요약/근거/변경내역).

### 어디에 쓰는가

`company_article_ai_state`의 `ai_summary`, `ai_snippets_json`, `ai_evidence_json`, `ai_structured_extraction_json` 매핑 원천.
...
`D:/intellij project/cherry-in-the-haystack/dev/apps/agent/news_agent/code/run_news_agent.py`

### 파이프라인 출력 JSON은 무엇인가

여러 기사에 대한 분류/점수/뉴스레터 적합성 판정 배치 결과.

### 파이프라인 출력 JSON 용도

`company_article_ai_state`의 `ai_summary`, `ai_score`, `ai_classification_json`, `ai_tags_json`, `ai_entities_json` 원천.
...
선택된 기사들을 묶어 사람이 읽는 뉴스레터 본문 형태로 생성한 문서 초안.

### 뉴스레터 초안 JSON 용도

발행/편집 워크플로우용 콘텐츠이며, 현재 DDL 즉시 적재 대상은 아님.
...
`D:/intellij project/cherry-in-the-haystack/dev/packages/pdf_knowledge_extractor/src/db/models.py`

### 이 JSON은 무엇인가

PDF 추출 파이프라인이 저장하는 원천 DB 엔터티 스키마 요약.

### 어디에 쓰는가

Writer의 `evidence/references` 생성을 위한 근거 데이터 소스(직접 `cherry_platform` 적재 아님).
...
`D:/intellij project/cherry-in-the-haystack/dev/packages/pdf_knowledge_extractor/src/workflow/nodes/process_section.py`

### 이 JSON은 무엇인가

섹션 처리 단계에서 LLM이 뽑는 개념 결과와 DB 저장 단위.

### 어디에 쓰는가

`paragraph_chunks`, `key_ideas` 적재 및 이후 기사 근거 추출의 업스트림 입력.
...
`cherry_platform` 스키마에서 요구하는 JSONB 컬럼 타입 계약.

### 어디에 쓰는가

에이전트 산출물 적재 시 최종 타깃 포맷 검증 기준.
...
- Writer Agent 결과 JSON
- News Agent 결과 JSON
- PDF extractor의 원천 DB write
...
에 **직접 INSERT/UPDATE 하는 변환 레이어(ETL)**