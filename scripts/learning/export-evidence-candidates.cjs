...
(async () => {
  ...
  /* 별칭도 검색어에 넣는다 — 책이 쓰는 표현과 온톨로지 이름이 다른 경우가 많다.
     세는 쪽(build-researcher-json)과 같은 SQL 을 공유 모듈에서 가져온다. */
  const { rows: als } = await c.query(ALIAS_SQL, [NODE]);
  const t = terms(cons[0].canonical_name, kids.map((k) => k.child), als.map((a) => a.alias_text));
  const re = toRegex(t);
