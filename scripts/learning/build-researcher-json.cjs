...
/* UI 메뉴 매핑 — 하드코딩하지 않는다. 정본은 apps/web/app/page.tsx 다(menu-map.cjs).
   ⚠️ 예전엔 여기 상수로 박혀 있었고, 2026-08-25 에 프론트만 고쳤더니 이 표가 옛 노드를
      가리킨 채 남아 pageType 이 틀리게 나왔다. 다시 상수로 되돌리지 말 것. */
const { menuByNode } = require("./menu-map.cjs");
const MENU = menuByNode();
if (!MENU) { console.error("중단: apps/web/app/page.tsx 의 CONCEPT_NODE_BY_TOPIC 을 읽지 못했습니다."); process.exit(1); }
...
(async () => {
 ...
 LEFT JOIN content.concept_page p ON p.ontology_node = c.ontology_node AND p.surface='learning'
  ...
  /* 별칭 — 개념별로 한 번에 담는다(개념마다 쿼리하지 않는다).
     검색어에 별칭이 빠지면 책이 쓰는 표현을 놓친다(2026-08-25 결함 수정). */
  const { rows: alrows } = await c.query(`
    SELECT c.ontology_node node, a.alias_text
      FROM handbook.concept_alias a
      JOIN handbook.concept c ON c.id = a.concept_id AND c.revoked_at IS NULL
     WHERE a.revoked_at IS NULL`);
  const aliasOf = {};
  alrows.forEach((r) => { (aliasOf[r.node] = aliasOf[r.node] || []).push(r.alias_text); });
 ...
 LEFT JOIN handbook.section s ON s.id=pc.section_id
  ...
  for (const row of concepts) {
    const kids = childrenOf[row.node] || [];
    const kidNames = kids.map((k) => k.child);
    /* 자기 별칭 + 하위 개념들의 별칭 */
    const aliasNames = [...(aliasOf[row.node] || []),
                        ...kids.flatMap((k) => aliasOf[k.child_node] || [])];
    const t = terms(row.name, kidNames, aliasNames);
